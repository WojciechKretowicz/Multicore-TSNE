...
void TSNE<treeT, dist_fn>::run(double* X, int N, int D, double* Y,double* steps,
               int no_dims,
    ...
    */
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        // remember next step
        for(int i=iter*N*no_dims;  i<(iter+1)*N*no_dims; i++)
            steps[i] =Y[i - iter*N*no_dims];
...
extern "C"
{
                                ...
                                double *final_error = NULL, int distance = 1)
    {
        ...
        if (distance == 0) {
            ...
            tsne.run(X, N, D, Y, steps, no_dims, perplexity, theta, num_threads, max_iter, n_iter_early_exag,
